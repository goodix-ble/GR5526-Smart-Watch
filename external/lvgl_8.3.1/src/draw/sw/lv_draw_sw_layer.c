...
extern void * app_graphics_mem_malloc(uint32_t size);
extern void   app_graphics_mem_free(void *buf_ptr);
#endif
...
#if GR552X_GPU_RENDER_SUPPORT > 0u
        layer_sw_ctx->base_draw.buf = app_graphics_mem_malloc(layer_sw_ctx->buf_size_bytes);
#else
...
#if GR552X_GPU_RENDER_SUPPORT > 0u
    draw_ctx->is_transform_buff = false;
	draw_ctx->is_blending_transform_buff = true;
#endif
    /*Blend the layer*/
    lv_draw_img(draw_ctx, draw_dsc, &layer_ctx->area_act, &img);
    lv_draw_wait_for_finish(draw_ctx);
#if GR552X_GPU_RENDER_SUPPORT > 0u
	draw_ctx->is_blending_transform_buff = false;
#endif
}
...
void lv_draw_sw_layer_destroy(lv_draw_ctx_t * draw_ctx, lv_draw_layer_ctx_t * layer_ctx)
{
    LV_UNUSED(draw_ctx);
#if GR552X_GPU_RENDER_SUPPORT > 0u
    app_graphics_mem_free((void*)layer_ctx->buf);
#else
