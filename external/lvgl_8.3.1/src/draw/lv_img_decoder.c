...
#undef  CF_BUILT_IN_LAST
#define CF_BUILT_IN_LAST LV_IMG_CF_GDX_A8
#endif
...
#if GR552X_GPU_IMG_SUPPORT > 0u
    else if(cf == LV_IMG_CF_GDX_RGB565 || 
            cf == LV_IMG_CF_GDX_RGB565_CHROMA_KEYED || 
            cf == LV_IMG_CF_GDX_RGBA8888 || 
            cf == LV_IMG_CF_GDX_RGBA8888_CHROMA_KEYED || 
            cf == LV_IMG_CF_GDX_TSC4 || 
            cf == LV_IMG_CF_GDX_TSC6a || 
            cf == LV_IMG_CF_GDX_A8) {
        if(dsc->src_type == LV_IMG_SRC_VARIABLE) {
            /*In case of uncompressed formats the image stored in the ROM/RAM.
             *So simply give its pointer*/
