...
lv_color_t lv_img_buf_get_px_color(lv_img_dsc_t * dsc, lv_coord_t x, lv_coord_t y, lv_color_t color)
{
    ...
    else if(dsc->header.cf == LV_IMG_CF_GDX_RGB565 || dsc->header.cf == LV_IMG_CF_GDX_RGB565_CHROMA_KEYED ||
       dsc->header.cf == LV_IMG_CF_GDX_RGBA8888 || dsc->header.cf == LV_IMG_CF_GDX_RGBA8888_CHROMA_KEYED) {
        uint8_t px_size = lv_img_cf_get_px_size(dsc->header.cf) >> 3;
        uint32_t px     = dsc->header.w * y * px_size + x * px_size;
        lv_memcpy_small(&p_color, &buf_u8[px], sizeof(lv_color_t));
#if LV_COLOR_SIZE == 32
        ...
        LV_LOG_ERROR("TODO: not handle %d in %s \r\n", dsc->header.cf, __FUNCTION__);
    }
#endif
    return p_color;
}
...
#if 0// GR552X_GPU_IMG_SUPPORT > 0u
    else {
        printf("TODO: not handle %d in %s \r\n", dsc->header.cf, __FUNCTION__);
    }
#endif
}
...
uint32_t lv_img_buf_get_img_size(lv_coord_t w, lv_coord_t h, lv_img_cf_t cf)
{
    switch(cf) {
        ...
        case LV_IMG_CF_GDX_RGB565:
            return LV_IMG_BUF_SIZE_INDEXED_8BIT(w, h)*2;
        ...
        case LV_IMG_CF_GDX_TSC6a:
            return (LV_IMG_BUF_SIZE_INDEXED_8BIT(w, h)*6/32);
#endif
        default:
            return 0;
    }
