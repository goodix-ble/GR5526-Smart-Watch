...
const uint8_t * lv_font_get_bitmap_fmt_txt(const lv_font_t * font, uint32_t unicode_letter)
{
    ...
    lv_font_fmt_txt_dsc_t * fdsc = (lv_font_fmt_txt_dsc_t *)font->origin_dsc;
#else
...
#if GR552X_GPU_FONT_SUPPORT
        uint32_t box_w = (gdsc->box_w % 2) ? (gdsc->box_w + 1) : gdsc->box_w;
        uint32_t gsize = box_w * gdsc->box_h;
#else
...
#if GR552X_GPU_FONT_SUPPORT
        decompress(&fdsc->glyph_bitmap[gdsc->bitmap_index], LV_GC_ROOT(_lv_font_decompr_buf), box_w, gdsc->box_h,
