...
{
#if LV_GDX_PATCH_STYLE_REFR_GATE_SEM
    static uint32_t style_refr_disable_cnt = 0;
    if (en)
    {
        if (style_refr_disable_cnt > 0)
        {
            style_refr_disable_cnt--;
        }
        style_refr = (style_refr_disable_cnt == 0);
    }
    else
    {
        style_refr_disable_cnt++;
