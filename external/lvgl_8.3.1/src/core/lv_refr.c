...
void _lv_disp_refr_timer(lv_timer_t * tmr)
{
    ...
    extern bool lv_wms_is_refresh_enabled(void);
    if(!lv_wms_is_refresh_enabled()) {
        return;
    }
#endif
...
void refr_obj(lv_draw_ctx_t * draw_ctx, lv_obj_t * obj)
{
    ...
    if(layer_type == LV_LAYER_TYPE_NONE) {
        ...
        draw_ctx->transform_buff_pixel_bytes = flags & LV_DRAW_LAYER_FLAG_HAS_ALPHA ? LV_IMG_PX_SIZE_ALPHA_BYTE : sizeof(lv_color_t);
#endif
