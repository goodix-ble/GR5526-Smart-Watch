...
void lv_canvas_fill_bg(lv_obj_t * canvas, lv_color_t color, lv_opa_t opa)
{
    ...
    else if((dsc->header.cf == LV_IMG_CF_GDX_RGB565) || (dsc->header.cf == LV_IMG_CF_GDX_RGBA8888) || (dsc->header.cf == LV_IMG_CF_TRUE_COLOR_ALPHA) ) {
        extern void lv_port_gpu_fill_bg(uint32_t fb_addr, uint16_t w, uint16_t h, uint32_t fb_format, uint32_t rgb);
        lv_port_gpu_fill_bg((uint32_t)dsc->data, dsc->header.w, dsc->header.h, dsc->header.cf, color.full);
    }
#endif
    else {
        uint32_t x;
