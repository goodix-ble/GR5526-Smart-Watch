...
static void draw_indic(lv_event_t * e)
{
    ...
    /**
     *  GOODIX: Since draw_mask is not implemented, use clip instead of mask
    **/
    lv_area_t mask_clip;
    const lv_area_t *clip_orig = draw_ctx->clip_area;
    lv_area_copy(&mask_clip, clip_orig);
    if (hor)
    {
        mask_clip.x2 = LV_MIN(mask_clip.x2, bar->indic_area.x2);
    }
    else
    {
        mask_clip.y1 = LV_MAX(mask_clip.y1, bar->indic_area.y1);
    }
    draw_ctx->clip_area = &mask_clip;
    lv_draw_rect(draw_ctx, &draw_rect_dsc, &mask_indic_max_area);
    draw_ctx->clip_area = clip_orig;
    draw_rect_dsc.border_opa = border_opa;
