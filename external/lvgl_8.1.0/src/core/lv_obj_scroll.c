...
void lv_obj_scroll_by(lv_obj_t * obj, lv_coord_t x, lv_coord_t y, lv_anim_enable_t anim_en)
{
    ...
    if(anim_en == LV_ANIM_ON) {
        ...
        if(x) {
            uint32_t t = lv_anim_speed_to_time((lv_disp_get_hor_res(d) * 2) << 8, 0, x);
        ...
        /***goodix: scroll should wait until animal finish*****/
        if((lv_anim_get(obj, scroll_y_anim) != NULL)|| (lv_anim_get(obj, scroll_x_anim) != NULL))
        {
            return;
        }
        /***goodix*******************************************/
