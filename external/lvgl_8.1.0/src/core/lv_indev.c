...
lv_obj_t * lv_indev_search_obj(lv_obj_t * obj, lv_point_t * point)
{
    ...
    if(lv_obj_hit_test(obj, point)) {
        #if GR552X_WMS_SERVICE_SUPPORT
        // Only search the root screen during view switching
        extern bool lv_wms_is_in_busy_state(void);
...
static void indev_proc_press(_lv_indev_proc_t * proc)
{
    ...
    /****goodix :don't process press event after switching scr ***/
    if (indev_obj_act != NULL){
        lv_obj_t* scr = indev_obj_act;
        while(scr->parent != NULL){
            scr = scr->parent;
        }
        if(scr != lv_scr_act()) return;
    }
    /****goodix :donot process press event after switching scr ***/
